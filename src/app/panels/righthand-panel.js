...
function RighthandPanel ({ api: {}, events: {} } = {}) {
  ...
  self._view.tabbedMenu = new TabbedMenu({ api: self._api, events: tabEvents })
  ...
  var compileTab = new CompileTab({ api: appAPI, events: events })
  optionViews.appendChild(compileTab.render())
  var runTab = new RunTab({ api: appAPI, events: events })
  optionViews.appendChild(runTab.render())
  var settingsTab = new SettingsTab({ api: appAPI, events: events })
  optionViews.appendChild(settingsTab.render())
  var analysisTab = new AnalysisTab({ api: appAPI, events: events })
  optionViews.appendChild(analysisTab.render())
  var debuggerTab = new DebuggerTab({ api: appAPI, events: events })
  optionViews.appendChild(debuggerTab.render())
  var supportTab = new SupportTab({ api: appAPI, events: events })
  ...
  self.pluginManager = new PluginManager({ api: appAPI, events: events })
  events.rhp.register('plugin-loadRequest', (json) => {
    var content = new PluginTab(json)
    this._view.tabbedMenu.addTab(json.title, 'plugin', content)
    self.pluginManager.register(json, content)
  })

  self.showDebugger = function () { appAPI.switchTab('debugView') }
